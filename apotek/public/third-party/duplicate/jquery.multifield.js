...
(function ($, window, document, undefined) {
    ...
    multiField.prototype = {
        ...
        cloneSection: function () {
            ...
            if (
                this.config.max !== 0 &&
                this.getSectionsCount() + 1 > this.config.max
            ) {
                return false;
            }

            // Clone last section
            var newChild = $(this.config.section, this.$elem)
                .last()
                .clone()
                .attr("style", "")
                .attr("id", "")
            ...
            newChild.find('input[type="radio"]').each(function () {
                ...
                $(this).attr(
                    "name",
                    name.replace(/([0-9]+)/g, 1 * name.match(/([0-9]+)/g) + 1)
                );
            });
        ...
        removeSection: function (section) {
            if (
                confirm(
                    this.localize_i18n.multiField.messages.removeConfirmation
                )
            ) {
